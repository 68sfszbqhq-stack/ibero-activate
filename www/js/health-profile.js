...
function calculateCurrentPhase(startDate) {
    ...
    let maxWeeks = 19;
    let phasesConfig = HEALTH_CONSTANTS.MACROCYCLE_PHASES;
    
    if (activePeriod === 'PRIMAVERA_2026') {
        const end = new Date('2026-05-22T23:59:59');
        if (today > end) referenceDate = end;
        maxWeeks = 19;
        phasesConfig = HEALTH_CONSTANTS.MACROCYCLE_PHASES;
    } else if (activePeriod === 'VERANO_2026') {
    ...
    const diffTime = referenceDate - start;
    const diffDays = Math.max(0, Math.ceil(diffTime / (1000 * 60 * 60 * 24)));
    const currentWeek = Math.min(Math.ceil(diffDays / 7), maxWeeks) || 1;

    // Determinar fase actual
    let phase = 1;
    for (const [phaseNum, phaseData] of Object.entries(phasesConfig)) {
    ...
    const phaseData = phasesConfig[phase] || phasesConfig[1];
    ...
    const colors = {
        ...
        3: activePeriod === 'VERANO_2026' ? 'linear-gradient(135deg, #8b5cf6, #7c3aed)' : 'linear-gradient(135deg, #f59e0b, #d97706)',
        4: 'linear-gradient(135deg, #8b5cf6, #7c3aed)'
    };

    const currentWeekInPhase = Math.max(1, currentWeek - phaseData.weeks[0] + 1);

    const phaseStartMs = start.getTime() + (phaseData.weeks[0] - 1) * 7 * 24 * 60 * 60 * 1000;
    const phaseEndMs = start.getTime() + phaseData.weeks[phaseData.weeks.length - 1] * 7 * 24 * 60 * 60 * 1000;

    return {
        phase: phase,
        currentWeek: currentWeek,
        currentWeekInPhase: currentWeekInPhase,
        totalWeeks: phaseData.weeks.length,
        ...
        color: colors[phase] || colors[1],
        startDate: new Date(phaseStartMs).toISOString(),
        endDate: new Date(phaseEndMs).toISOString()
    };
}
    ...
    try {
        ...
        if (profile.macrocycle) {
            ...
            if (activePeriod === 'VERANO_2026') {
                startDateVal = new Date('2026-06-01T00:00:00');
            } else {
                startDateVal = profile.macrocycle.start_date ? profile.macrocycle.start_date.toDate() : new Date('2026-01-12T00:00:00');
            }
            const currentPhase = calculateCurrentPhase(startDateVal);
